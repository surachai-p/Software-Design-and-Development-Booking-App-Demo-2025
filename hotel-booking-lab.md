...
[บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/1.png)
...
>[บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/2.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/3.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/4.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/5.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/6.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/7.png)
...
>[บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/8.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/9.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/10.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/11.png)
...
> [บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](photo/12.png)
...
```
Axios เป็นไลบรารีสำหรับส่ง HTTP request ใน JavaScript ที่ทำให้การเรียก API ง่ายกว่า fetch เพราะมีฟีเจอร์ต่างๆ เช่น automatic JSON parsing, error handling ที่ดีกว่า, และสามารถตั้งค่า global config ได้
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
เพื่อรอให้ AuthContext โหลดข้อมูล user จาก localStorage เสร็จก่อน ถ้าไม่ตรวจสอบอาจ redirect ไปหน้า login ทันทีแม้ว่าจะมีข้อมูลอยู่ก็ตาม ทำให้ผู้ใช้ต้อง login ใหม่ทุกครั้งที่ refresh หน้า
```

**คำถามที่ 3:** `localStorage` ที่ใช้เก็บ JWT token มีความเสี่ยงด้านความปลอดภัยอย่างไร และมีวิธีอื่นที่ดีกว่าหรือไม่?

```
localStorage มีความเสี่ยงเพราะทุก script ในหน้าเว็บสามารถเข้าถึงได้ รวมถึง XSS attacks และไม่มี expiration. วิธีที่ดีกว่าคือใช้ httpOnly cookies หรือเก็บ token ใน memory เท่านั้น และใช้ refresh token
```
