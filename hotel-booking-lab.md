...
> แทรกรูปภาพที่นี่ ![alt text](images/run.png)
...
> แทรกรูปภาพที่นี่ ![alt text](images/b.png) ![alt text](images/b1.png) ![alt text](images/b2.png)
...
> แทรกรูปภาพที่นี่ ![alt text](images/admin.png) ![alt text](images/admin1.png)
...
> แทรกรูปภาพที่นี่ ![alt text](images/admin2.png)

### 📸 บันทึกผลการทดลอง: หน้าแก้ไขข้อมูลการจอง

> แทรกรูปภาพที่นี่ ![alt text](images/edit.png) ![alt text](images/edit1.png) ![alt text](images/d2.png) ![alt text](images/d3.png)
...
> แทรกรูปภาพที่นี่ ![alt text](images/out1.png) ![alt text](images/out.png)
...
```
Axios คือ Library ที่ช่วยจัดการ HTTP Request ด้วยฟีเจอร์ที่ครบเครื่อง ทั้งการแปลง JSON อัตโนมัติและการใช้ Interceptor เพื่อจัดการ Token ในส่วนกลาง

ในขณะที่ Fetch API เป็นเครื่องมือมาตรฐานที่มาพร้อมกับ Browser ซึ่งเน้นความเบาและเรียบง่าย แต่ผู้พัฒนาต้องเขียน Logic เพิ่มเพื่อเช็ค HTTP Error และแปลงข้อมูลเอง

การเลือกใช้จึงขึ้นอยู่กับขนาดของโปรเจกต์ หากเป็นแอปพลิเคชันที่ซับซ้อน Axios จะช่วยลดภาระในการเขียนโค้ดได้มากกว่า แต่หากเน้นความคล่องตัวและไม่ต้องการลง Library เพิ่ม Fetch ก็เป็นตัวเลือกที่ดี
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
1.เพื่อป้องกันระบบดีดผู้ใช้กลับไปหน้า Login โดยไม่จำเป็น (False Redirect) ในขณะที่แอปกำลังรอตรวจสอบสถานะหรือดึงข้อมูลผู้ใช้จาก API

2.ช่วยแก้ไขปัญหาหน้าเว็บกระพริบ (Flash of Unauthenticated Content) ซึ่งทำให้ผู้ใช้เห็นหน้าอื่นแวบหนึ่งก่อนที่ข้อมูลยืนยันตัวตนจะโหลดเสร็จ

3.ป้องกันแอปพัง (Crash) จากการพยายามเรียกใช้ข้อมูลผู้ใช้ในหน้าถัดไปที่ยังมีค่าเป็น Null หรือ Undefined ในระหว่างที่กระบวนการโหลดยังไม่เสร็จสิ้น
```

**คำถามที่ 3:** `localStorage` ที่ใช้เก็บ JWT token มีความเสี่ยงด้านความปลอดภัยอย่างไร และมีวิธีอื่นที่ดีกว่าหรือไม่?

```
การเก็บ JWT ใน localStorage เสี่ยงต่อการถูกขโมยข้อมูลผ่านการโจมตีแบบ XSS (Cross-Site Scripting) ซึ่งแฮกเกอร์สามารถรัน Script เพื่อดึง Token ออกไปได้โดยง่าย

วิธีที่ปลอดภัยกว่าคือการใช้ HttpOnly Cookie เพราะ Browser จะไม่อนุญาตให้ JavaScript เข้าถึงไฟล์ Cookie นี้ได้ จึงช่วยปิดช่องโหว่จากการถูกดึง Token ไปโดยตรง
```
