...
// GET /api/bookings/:id (ต้อง Login)
...
// POST /api/bookings (สร้างการจอง - Public)
app.post('/api/bookings', (req, res) => {
  const { fullname, email, phone, checkin, checkout, roomtype, guests } = req.body;
  if (!fullname) return res.status(400).json({ error: "ต้องระบุ fullname" });

  const sql = `INSERT INTO bookings (fullname, email, phone, checkin, checkout, roomtype, guests, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')`;
...
// DELETE /api/bookings/:id (ต้อง Login)
app.delete('/api/bookings/:id', authenticateToken, (req, res) => {
  db.run('DELETE FROM bookings WHERE id = ?', [req.params.id], function(err) {
    if (err) return res.status(400).json({ error: err.message });
    if (this.changes === 0) return res.status(404).json({ error: 'ไม่พบข้อมูล' });
    
    // ส่ง Response ตามเงื่อนไขพิเศษของคุณ
    res.json({ 
      status: "ลบข้อมูลสำเร็จโดย Chitsanupat Wayusuwanwit" 
    });
  });
});
