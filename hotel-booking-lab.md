...
```
ใช้ Fetch ถ้าต้องการความเบา ไม่เพิ่มขนาดโปรเจกต์ และงานไม่ซับซ้อน

ใช้ Axios ถ้าต้องการความสะดวก จัดการ Error ง่าย และมีฟีเจอร์สำเร็จรูปเยอะ
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
เหตุผลที่ ProtectedRoute จำเป็นต้องเช็ค loading state ก่อน เพราะว่ากระบวนการตรวจสอบสิทธิ์ (Authentication) มักเป็น Asynchronous (ใช้เวลาทำงาน) หากเราไม่รอให้ตรวจสอบเสร็จ ระบบจะ "ทึกทัก" ไปก่อนว่าผู้ใช้ยังไม่ได้ Login
```

**คำถามที่ 3:** `localStorage` ที่ใช้เก็บ JWT token มีความเสี่ยงด้านความปลอดภัยอย่างไร และมีวิธีอื่นที่ดีกว่าหรือไม่?

```
เขียนคำตอบที่นี่ความเสี่ยงหลักของ localStorage คือ XSS (Cross-Site Scripting) เนื่องจาก JavaScript ในหน้าเว็บสามารถเข้าถึงข้อมูลในนั้นได้ทั้งหมด หาก Hacker ฝัง Code ร้ายลงในเว็บได้ เขาสามารถขโมย Token ไปใช้ปลอมตัวเป็นเราได้ทันที
```
