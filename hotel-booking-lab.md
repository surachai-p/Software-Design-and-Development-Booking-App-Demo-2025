...
```
เขียนคำตอบที่นี่ คือ ไลบรารี (Library) ภายนอกที่ใช้สำหรับส่ง HTTP Request ไปยัง API หรือเซิร์ฟเวอร์ ทำให้เขียนโค้ดจัดการข้อมูลแบบ Asynchronous ได้ง่ายขึ้น
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
เขียนคำตอบที่นี่ ถ้าไม่มี loading เราจะต้องเหนื่อยพิมพ์รหัสผ่านใหม่ "ทุกครั้ง" ที่กดรีเฟรชหน้าเว็บ
```

**คำถามที่ 3:** `localStorage` ที่ใช้เก็บ JWT token มีความเสี่ยงด้านความปลอดภัยอย่างไร และมีวิธีอื่นที่ดีกว่าหรือไม่?

```
เขียนคำตอบที่นี่ การเก็บ JWT Token ไว้ใน localStorage มีความเสี่ยงตรงที่ โค้ด JavaScript ทุกตัวบนหน้าเว็บสามารถเข้าถึงและอ่านข้อมูลส่วนนี้ได้อย่างอิสระ
```
