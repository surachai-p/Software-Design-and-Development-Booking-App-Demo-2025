...
// IMPROVED: Custom response as per lab requirements
app.delete('/api/bookings/:id', authenticateToken, (req, res) => {
  db.run('DELETE FROM bookings WHERE id = ?', [req.params.id], function(err) {
    if (err)             return res.status(400).json({ error: err.message });
    if (this.changes === 0) return res.status(404).json({ error: 'ไม่พบข้อมูลการจอง' });
    res.json({ status: "ลบข้อมูลสำเร็จโดย Wasuratsomdet", id: req.params.id });
  });
});

// GET /api/users — ดูรายการ user ทั้งหมด (ต้อง authenticateToken)
// IMPROVED: Added as per lab requirements, hides password
