...
app.delete('/api/bookings/:id', authenticateToken, (req, res) => {
  ...
  db.run(`DELETE FROM bookings WHERE id = ?`, id, function(err) {
    ...
    // 👇 แก้ไขบรรทัดนี้บรรทัดเดียวเลยครับ 👇
    res.json({ status: "ลบข้อมูลสำเร็จโดย นาย/นางสาว [ใส่ชื่อนามสกุลของคุณ]" });
  });
});

// ดึงข้อมูลผู้ใช้งานทั้งหมด (GET /api/users)
app.get('/api/users', authenticateToken, (req, res) => {
  // สังเกตคำสั่ง SQL: เราเลือกมาแค่ id, username, role (ตั้งใจไม่ดึง password มา)
