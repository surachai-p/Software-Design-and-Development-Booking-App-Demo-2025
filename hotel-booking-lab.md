...
> แทรกรูปภาพที่นี่
![ผลการรัน Backend Server](<image/ผลการรัน Backend Server.png>)
---
...
> แทรกรูปภาพที่นี่
![ผลการทดสอบ Login และ Token](<image/Login และ Token.png>)
---
...
> แทรกรูปภาพที่นี่
![ผลการเพิ่มข้อมูลการจอง (POST) 3 รายการ](<image/(POST) 3 รายการ.png>)
---
...
![ผลการ GET ข้อมูลทั้งหมด](image/ดึงข้อมูลทั้งหมด.png)
---
...
> แทรกรูปภาพที่นี่
![ผลการ GET ข้อมูลโดยระบุ ID](<image/ดึงข้อมูลตาม ID .png>)
---
...
> แทรกรูปภาพที่นี่
![ผลการแก้ไขข้อมูล](<image/แก้ไขข้อมูลการจอง .png>)
---
...
> แทรกรูปภาพที่นี่
![ผลการลบข้อมูล](image/ลบข้อมูลการจอง.png)
---
...
> แทรกรูปภาพที่นี่
![ผลการทดลอง](<image/การ DELETE with custom status.png>)

![ผลการทดลอง](<image/GET :api:users.png>)
---
...
> แทรกรูปภาพที่นี่
![ผลการรัน Frontend เริ่มต้น](<image/Frontend เริ่มต้น.png>)
---
...
> แทรกรูปภาพที่นี่
![ผลการทดลอง](image/หน้าฟอร์มจองห้องพัก.png)
![ผลการทดลอง](image/หน้าสรุปการจอง.png)
...
> แทรกรูปภาพที่นี่
![ผลการทดลอง](<image/หน้า Login.png>)
![ผลการทดลอง](image/AdminDashboard.png)
---
...
> แทรกรูปภาพที่นี่
![ผลการทดลอง](image/หน้ารายการจองห้องพัก.png)

### 📸 บันทึกผลการทดลอง: หน้าแก้ไขข้อมูลการจอง
> แทรกรูปภาพที่นี่
![ผลการทดลอง](image/หน้าแก้ไขข้อมูลการจอง.png)
---
...
> แทรกรูปภาพที่นี่
![ผลการทดลอง](<image/Logout และ Protected Route.png>)
---
...
```
Axios คือ Library ภายนอกที่ใช้ในการส่ง HTTP Request เพื่อเชื่อมต่อสื่อสารระหว่าง Frontend และ Backend ส่วน Fetch คือฟังก์ชันมาตรฐานที่มีมาให้ในตัว Browser (Native API) เลยครับ

ความแตกต่างที่สำคัญ:

การจัดการข้อมูล: Axios จะแปลงข้อมูลเป็น JSON ให้เราโดยอัตโนมัติ แต่ Fetch ต้องสั่ง .json() เองอีกขั้นตอนหนึ่ง

การจัดการ Error: Axios จะ "ดีด" ไปที่ส่วน catch (Error) ทันทีหาก Server ตอบกลับมาด้วย Status Code ที่ไม่สำเร็จ (เช่น 404 หรือ 500) แต่ Fetch จะมองว่าสำเร็จตราบเท่าที่ยังเชื่อมต่อได้ เราต้องเขียน if (!response.ok) เพื่อเช็คเอง

ความสะดวก: Axios มีฟีเจอร์เสริมมากกว่า เช่น การตั้งค่า Timeout, การใช้ Interceptors (ดักจับ Request/Response) และรองรับ Browser รุ่นเก่าได้ดีกว่า
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
เนื่องจากการตรวจสอบสถานะการเข้าสู่ระบบ (เช่น การเช็ค Token ใน localStorage) ต้องใช้เวลาเสร็จสิ้นในระดับมิลลิวินาทีตอนที่แอปพลิเคชันเริ่มทำงาน

ถ้าไม่ตรวจสอบ loading state จะเกิดอะไรขึ้น:
ระบบจะทำการตัดสินใจทันทีในขณะที่ข้อมูล Token ยังโหลดมาไม่เสร็จ ทำให้ตัวแปรที่เก็บสถานะการ Login (เช่น user หรือ isAuthenticated) ยังคงเป็นค่าว่าง (null หรือ false) ผลก็คือ ผู้ใช้จะถูกดีดกลับไปหน้า Login โดยอัตโนมัติ (Redirect) ทั้งที่จริงๆ แล้วเขาล็อกอินอยู่แล้ว เพียงเพราะโค้ดทำงานเร็วกว่าการดึงข้อมูลนั่นเองครับ
```
...
```
ความเสี่ยงหลัก:
localStorage มีความเสี่ยงต่อการถูกโจมตีแบบ XSS (Cross-Site Scripting) ครับ หากผู้ไม่หวังดีสามารถรันสคริปต์ JavaScript บนหน้าเว็บของเราได้ (เช่น ผ่านช่องทางคอมเมนต์หรือฟอร์มที่ไม่ปลอดภัย) เขาจะสามารถใช้คำสั่ง localStorage.getItem('token') เพื่อขโมย Token ของผู้ใช้ไปได้ง่ายๆ

วิธีอื่นที่ดีกว่า:

HttpOnly Cookie: การเก็บ Token ไว้ใน Cookie ที่ตั้งค่า HttpOnly จะทำให้ JavaScript ไม่สามารถเข้าถึง Cookie นั้นได้เลย ช่วยป้องกันการขโมย Token ผ่าน XSS ได้ดีที่สุด

Memory Storage: เก็บไว้ในตัวแปรธรรมดาในหน่วยความจำของแอป (แต่ถ้า Refresh หน้าเว็บ ข้อมูลจะหาย ต้องใช้ Refresh Token มาช่วยดึงใหม่)
```
