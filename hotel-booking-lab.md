...
```
axios เป็นตัวช่วยส่งข้อมูลระหว่างหน้าบ้านกับหลังบ้านครับ ต่างจาก fetch ตรงที่มัน ฉลาดกว่า เพราะมันจะแปลงข้อมูลเป็น JSON ให้เราเองเลย ไม่ต้องเขียนโค้ดแปลงหลายรอบเหมือน fetch แถมถ้าเซิร์ฟเวอร์ตอบกลับมาเป็น Error (เช่น 404 หรือ 500) axios จะแจ้งเตือนทันที ทำให้เราเขียนดักบั๊กได้ง่ายกว่าครับ
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
เพราะเวลาเราเปิดหน้าเว็บ ระบบมันต้องใช้เวลาเสี้ยววินาทีเพื่อไปเช็คใน localStorage ว่าเรา Login ค้างไว้ไหม ถ้าเราไม่สั่งให้มัน "รอ" (loading) ก่อน ระบบมันจะนึกว่าเรายังไม่ได้ Login แล้วดีดเรากลับไปหน้า Login ทันที ทั้งที่จริงๆ เราแค่กำลังโหลดข้อมูลอยู่ครับ
```

**คำถามที่ 3:** `localStorage` ที่ใช้เก็บ JWT token มีความเสี่ยงด้านความปลอดภัยอย่างไร และมีวิธีอื่นที่ดีกว่าหรือไม่?

```
เสี่ยงตรงที่ JavaScript ตัวไหนในเครื่องก็อ่านค่าได้ครับ ถ้าเราไปเผลอลง Script ปลอมหรือโดนดักหน้าเว็บ (XSS) Hacker จะขโมย Token เราไปใช้ได้ง่ายๆ เลย
วิธีที่ดีกว่า: คือการเก็บใน HttpOnly Cookie ครับ เพราะวิธีนี้จะทำให้ JavaScript เข้าถึง Token ไม่ได้เลย ปลอดภัยกว่าการเก็บไว้ใน localStorage แบบที่ทำอยู่ตอนนี้ครับ
```
