...
```
เขียนคำตอบที่นี่
การติดตั้ง: Axios ต้องลงเพิ่มผ่าน npm ส่วน Fetch มีให้ใช้ในตัวเบราว์เซอร์เลย

การแปลงข้อมูล: Axios แปลงเป็น JSON ให้โดยอัตโนมัติ แต่ Fetch ต้องสั่ง .json() เอง

การจัดการ Error: Axios จะแจ้ง Error ทันทีเมื่อเซิร์ฟเวอร์ตอบกลับรหัส 4xx หรือ 5xx ขณะที่ Fetch จะไม่มองว่าเป็น Error จนกว่าการเชื่อมต่อจะหลุด

ความสะดวก: Axios เขียนโค้ดสั้นและอ่านง่ายกว่าในการใช้งานจริง

ฟีเจอร์: Axios รองรับการตั้งค่าที่ซับซ้อน เช่น Timeout หรือ Interceptor ได้ง่ายกว่า
```
...
```
เขียนคำตอบที่นี่
เหตุผลที่ ProtectedRoute ต้องตรวจสอบ loading state ก่อน เป็นเพราะ กระบวนการอ่านข้อมูลจาก localStorage ต้องใช้เวลาเล็กน้อย หากไม่รอให้โหลดเสร็จก่อน แอปพลิเคชันจะเข้าใจผิดว่าไม่มีผู้ใช้งานล็อกอินอยู่
```
...
```
เขียนคำตอบที่นี่
ช่องโหว่ XSS (Cross-Site Scripting): นี่คือความเสี่ยงหลัก เนื่องจาก JavaScript ในหน้าเว็บสามารถเข้าถึงข้อมูลใน localStorage ได้ทั้งหมด หากแฮกเกอร์สามารถฝังโค้ดอันตรายลงในเว็บได้ (เช่น ผ่านคอมเมนต์หรือ URL) เขาสามารถสั่ง localStorage.getItem('token') เพื่อขโมย Token ของผู้ใช้ไปได้ทันที

ข้อมูลไม่ถูกลบเมื่อปิดเบราว์เซอร์: ข้อมูลใน localStorage จะคงอยู่ตลอดไปจนกว่าจะสั่งลบหรือล้างแคช หากผู้ใช้ลืม Logout ในเครื่องสาธารณะ ผู้อื่นที่มาใช้ต่ออาจเข้าถึงระบบได้
```
