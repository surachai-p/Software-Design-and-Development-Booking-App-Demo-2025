...
```Axios คือไลบรารีสำหรับใช้ส่ง HTTP Request จากฝั่ง Client ไปยัง Server เช่น GET, POST, PUT และ DELETE เพื่อเรียกใช้ API ต่าง ๆ โดย Axios ถูกพัฒนาขึ้นเพื่อให้การทำงานกับ API ทำได้ง่ายและสะดวกมากขึ้น
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
ProtectedRoute ต้องตรวจสอบ loading state ก่อน เนื่องจากในช่วงที่แอปเริ่มต้นทำงาน ระบบกำลังโหลดข้อมูลผู้ใช้และ token จาก localStorage เพื่อตรวจสอบว่าผู้ใช้ได้ Login อยู่หรือไม่

หากไม่ตรวจสอบ loading state จะเกิดปัญหาดังนี้

React อาจตรวจสอบ user ก่อนที่ข้อมูลจะโหลดเสร็จ

ค่า user จะยังเป็น null

ระบบจะเข้าใจว่าผู้ใช้ ยังไม่ได้ Login

ทำให้ redirect ไปหน้า Login ทั้งที่จริง ๆ ผู้ใช้ Login อยู่แล้ว
```

**คำถามที่ 3:** `localStorage` ที่ใช้เก็บ JWT token มีความเสี่ยงด้านความปลอดภัยอย่างไร และมีวิธีอื่นที่ดีกว่าหรือไม่?

```
การเก็บ JWT token ใน localStorage มีความเสี่ยงด้านความปลอดภัย เนื่องจาก JavaScript ที่รันในหน้าเว็บสามารถเข้าถึง localStorage ได้ หากเว็บไซต์มีช่องโหว่ประเภท Cross-Site Scripting (XSS) ผู้โจมตีอาจสามารถดึง JWT token ไปใช้งานแทนผู้ใช้ได้
ดังนั้นแม้ว่า localStorage จะใช้งานง่ายและนิยมในโปรเจคขนาดเล็ก แต่สำหรับระบบที่ต้องการความปลอดภัยสูง มักนิยมใช้ HTTP-only cookies ร่วมกับระบบ refresh token แทน
```
