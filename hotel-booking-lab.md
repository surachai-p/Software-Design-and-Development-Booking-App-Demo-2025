...
> แทรกรูปภาพที่นี่ ![alt text](images/4.png)
...
```
Axios คือไลบรารีเสริมที่ช่วยให้การเชื่อมต่อ API ง่ายขึ้น โดยต่างจาก fetch (คำสั่งพื้นฐานในเบราว์เซอร์) ตรงที่ Axios จะแปลงข้อมูลเป็น JSON ให้เองอัตโนมัติ และดักจับ Error (เช่น 404, 500) เข้าบล็อก catch ให้ทันที ทำให้เขียนโค้ดสั้นและจัดการข้อผิดพลาดได้ง่ายกว่า นอกจากนี้ยังมีระบบ Interceptors ที่ช่วยแนบ Token ลงใน Header ได้อัตโนมัติทุกครั้งที่ส่ง Request ในขณะที่ fetch ต้องเขียนจัดการสิ่งเหล่านี้เองทั้งหมดด้วยมือ
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
เพราะการตรวจสอบสิทธิ์หรือดึงข้อมูลผู้ใช้จาก Token ต้องใช้เวลาประมวลผลเล็กน้อย การเช็ก loading จึงเป็นการบอกให้ระบบ "รอ" จนกว่าจะรู้ผลลัพธ์ที่แน่นอนว่าผู้ใช้ล็อกอินอยู่จริงหรือไม่
ถ้าไม่ตรวจสอบระบบจะตัดสินไปเองทันทีว่าผู้ใช้ยังไม่ได้ล็อกอิน (เพราะข้อมูลผู้ใช้ยังโหลดมาไม่ถึงและยังเป็นค่าว่างอยู่) ส่งผลให้ผู้ใช้ที่ล็อกอินอยู่แล้วถูกดีดกลับไปหน้า Login โดยอัตโนมัติ หรือเกิดอาการหน้าเว็บกะพริบสลับไปมา ทำให้ใช้งานไม่ได้ตามปกติ
```

**คำถามที่ 3:** `localStorage` ที่ใช้เก็บ JWT token มีความเสี่ยงด้านความปลอดภัยอย่างไร และมีวิธีอื่นที่ดีกว่าหรือไม่?

```
ความเสี่ยงlocalStorage มีความเสี่ยงหลักต่อการถูกโจมตีแบบ XSS (Cross-Site Scripting) เนื่องจากโค้ด JavaScript ใดๆ ที่รันบนหน้าเว็บสามารถเข้าถึงและขโมย Token ออกไปได้ง่าย หากแฮกเกอร์ฝังโค้ดอันตรายลงในเว็บเราได้ เขาก็จะสามารถสวมรอยเป็นผู้ใช้คนนั้นได้ทันที
วิธีที่ดีกว่าคือการใช้ HttpOnly Cookie ในการเก็บ Token แทน เพราะคุกกี้ประเภทนี้จะถูกจัดการโดยเบราว์เซอร์และฝั่งเซิร์ฟเวอร์เท่านั้น ทำให้ JavaScript ฝั่งหน้าบ้านไม่สามารถอ่านหรือเขียนข้อมูลได้ จึงช่วยป้องกันการขโมย Token ผ่าน XSS ได้อย่างมีประสิทธิภาพ
```
...
- [ ✓ ] รูปผลการทดลองตามที่กำหนดในแต่ละขั้นตอน ([แทรกในไฟล์นี้](images))
