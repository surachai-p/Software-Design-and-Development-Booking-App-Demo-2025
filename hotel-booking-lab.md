...
```
axios คือ library ที่เอาไว้ส่ง HTTP request ไปที่ sever เพื่อดึงข้อมูล API
ความแตกต่างคือ axios ใช้งานง่ายกว่า โดยสามารถแปลงข้อมูล json และจัดการตัว errorที่สะดวกกว่า ส่วน fetch มีอยู่ใน Javascript อยู่แล้วไม่ต้องติดตั้งเพิ่ม แต่ต้องเขียนโค้ดเพื่อจัดการ error เพิ่มเติม
```

**คำถามที่ 2:** เหตุใด `ProtectedRoute` จึงต้องตรวจสอบ `loading` state ก่อน ถ้าไม่ตรวจสอบจะเกิดอะไรขึ้น?

```
เพื่อให้แน่ใจว่าระบบตรวจสอบการ login ของผู้ใช้เสร็จแฃ้ว หากไม่ตรวจสอบก่อนอาจะทำให้ระบบ redirect ไปหน้า login ทันที ทั้งที่ผู้ใช้งานอาจยัง login อยู่ ส่งผลให้เกิดการผิดพลาดและทำให้หน้าเว็บโหลดซ้ำหลายครั้ง
```

**คำถามที่ 3:** `localStorage` ที่ใช้เก็บ JWT token มีความเสี่ยงด้านความปลอดภัยอย่างไร และมีวิธีอื่นที่ดีกว่าหรือไม่?

```
มีความเสี่ยงคือ หากเว็บมีช่องโหว่ ผู้โจมตีอาจสามารถเข้าถึง token และนำไปใช้แทนตัวปู้ใช้ได้ วิธีที่ปลอดภัยกว่าคือ เก็บtoken ในHttpOnly cookie ซึ่ง Javascript ไม่สามารถเข้าถึงได้ ก็จะลดความเสี่ยงไปได้
```
