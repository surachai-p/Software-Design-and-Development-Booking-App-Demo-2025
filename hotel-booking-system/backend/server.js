...
app.delete('/api/bookings/:id', authenticateToken, (req, res) => {
  db.run('DELETE FROM bookings WHERE id = ?', [req.params.id], function(err) {
    ...
    // ตรงนี้คือส่วนที่อาจารย์ให้แก้
    res.json({ status: 'ลบข้อมูลสำเร็จโดย ธนภัทร' }); 
  });
});
// GET /api/users — ดึงข้อมูลผู้ใช้ทั้งหมดแบบไม่แสดงรหัสผ่าน (งานที่อาจารย์สั่งเพิ่ม)
app.get('/api/users', authenticateToken, (req, res) => {
  // สังเกตว่าใน SELECT เราจะไม่ดึงคอลัมน์ password มา
